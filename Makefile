PROJECT_NAME ?= disc_backend
VERSION = $(shell python3 setup.py --version | tr '+' '-')
PROJECT_NAMESPACE ?= dvurakov
...
postgres:
	docker stop disc-backend-postgres || true
	docker run --rm --detach --name=disc-backend-postgres \
		--env POSTGRES_USER=dvurakov \
		--env POSTGRES_PASSWORD=andNothingCanGoWrong \
		--env POSTGRES_DB=backend_school \
...
docker: sdist
	docker build --platform linux/amd64 --target=api -t $(PROJECT_NAME):$(VERSION) .

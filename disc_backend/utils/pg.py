...
CENSORED = 'andNothingCanGoWrong'
DEFAULT_PG_URL = 'postgres://dvurakov:andNothingCanGoWrong@localhost/backend_school'
    ...
    async def __aiter__(self):
        async with self.transaction_ctx as conn:
            ...
            async for row in cursor:
                row = dict(row)
                row['updateDate'] = datetime.strftime(row['updateDate'], '%Y-%m-%dT%H:%M:%S') + '.000Z'
                yield dict(row)


class HierarchyQuery(AsyncIterable):
    """
    Используется, чтобы отправлять данные из PostgreSQL клиенту сразу после
    получения, по частям, без буфферизации всех данных.
    """
